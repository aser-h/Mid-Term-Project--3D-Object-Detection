...
def show_pcl(pcl):
    ...
    viz.create_window(window_name='pcl', width=900, height=900, left=50, top=50, visible=True)
    ...
    viz.register_key_callback(262, next_slide)
    viz.run()
...
def show_range_image(frame, lidar_name):
    ...
    intensity_image_norm = cv2.convertScaleAbs(intensity_image_norm, alpha=(255/(p99-p1)), beta=(-255*p1/(p99-p1)))

    # step 6 : stack the range and intensity image vertically using np.vstack and convert the result to an unsigned 8-bit integer  
    deg90 = int(intensity_image_norm.shape[1] / 4)
    ri_center = int(intensity_image_norm.shape[1]/2)
    intensity_image_norm = intensity_image_norm[:,ri_center-deg90:ri_center+deg90]
    img_range_intensity = np.vstack((img_range, intensity_image_norm))
...
def bev_from_pcl(lidar_pcl, configs, vis=True):
    ...
    _, indexes, counter = np.unique(lidar_pcl_cpy[:, 0:2], axis=0, return_index=True, return_counts=True)
    ...
    indices = (np.int_(lidar_pcl_top[:, 0]), np.int_(lidar_pcl_top[:, 1]))
    height_values = lidar_pcl_top[:, 2] / float(np.abs(configs.lim_z[1] - configs.lim_z[0]))
    height_map[indices] = height_values
